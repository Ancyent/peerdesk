...
const SectionTitle = ({ children }: { children: string }) => (
  <div style={{ fontSize: 10, color: '#8b949e', letterSpacing: 1, fontWeight: 600, marginBottom: 10, paddingBottom: 6, borderBottom: '1px solid #21262d', textTransform: 'uppercase' }}>{children}</div>
);
    ...
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {MODES.map(m => (
          <div key={m.value} onClick={() => updateSetting('access_mode', m.value)} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10, cursor: 'pointer' }}>
            ...
            <div>
              <div style={{ fontSize: 12, color: '#c9d1d9' }}>{m.label}</div>
              <div style={{ fontSize: 10, color: '#8b949e', marginTop: 2 }}>{m.desc}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {([
          ...
          <div key={item.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #1c2128' }}>
            <div>
              ...
              {item.desc && <div style={{ fontSize: 10, color: '#8b949e' }}>{item.desc}</div>}
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0' }}>
          <div>
            <div style={{ fontSize: 12, color: '#c9d1d9' }}>Auto-disconnect on inactivity</div>
            <div style={{ fontSize: 10, color: '#8b949e' }}>Close session after idle</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {settings.auto_disconnect_minutes !== null && (
              <select value={settings.auto_disconnect_minutes ?? 10} onChange={e => updateSetting('auto_disconnect_minutes', Number(e.target.value))} style={{ background: '#21262d', border: '1px solid #30363d', borderRadius: 5, padding: '3px 6px', fontSize: 11, color: '#c9d1d9' }}>
