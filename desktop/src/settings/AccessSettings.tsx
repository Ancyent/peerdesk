...
export function AccessSettings({ settings, updateSetting }: Props) {
  ...
  const MODES = [
    { value: 'full' as const, label: t('settings:access.modes.full.label'), desc: t('settings:access.modes.full.desc') },
    { value: 'view_only' as const, label: t('settings:access.modes.viewOnly.label'), desc: t('settings:access.modes.viewOnly.desc') },
    { value: 'no_incoming' as const, label: t('settings:access.modes.noIncoming.label'), desc: t('settings:access.modes.noIncoming.desc') },
  ];

  return (
    <div style={{ padding: '20px 24px' }}>
      <div style={{ fontSize: 15, fontWeight: 600, color: '#e6edf3', marginBottom: 20 }}>{t('settings:access.title')}</div>

      <div style={{ marginBottom: 20 }}>
        <SectionTitle>{t('settings:access.sectionAccessMode')}</SectionTitle>
        {MODES.map(m => (
      ...
      <div style={{ marginBottom: 20 }}>
        <SectionTitle>{t('settings:access.sectionIncomingRequests')}</SectionTitle>
        {([
          { key: 'show_approval_dialog' as const, label: t('settings:access.showApprovalDialog.label'), desc: t('settings:access.showApprovalDialog.desc') },
          { key: 'lock_screen_after_session' as const, label: t('settings:access.lockScreenAfterSession.label'), desc: '' },
        ] as const).map(item => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0' }}>
          <div>
            <div style={{ fontSize: 12, color: '#e6ebf1' }}>{t('settings:access.autoDisconnect.label')}</div>
            <div style={{ fontSize: 10, color: '#b3bdca' }}>{t('settings:access.autoDisconnect.desc')}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {settings.auto_disconnect_minutes !== null && (
              <select value={settings.auto_disconnect_minutes ?? 10} onChange={e => updateSetting('auto_disconnect_minutes', Number(e.target.value))} style={{ background: '#21262d', border: '1px solid #30363d', borderRadius: 5, padding: '3px 6px', fontSize: 11, color: '#e6ebf1' }}>
                {[5, 10, 15, 30].map(n => <option key={n} value={n}>{t('settings:access.autoDisconnect.minutes', { n })}</option>)}
              </select>
            )}
      ...
      <div>
        <SectionTitle>{t('settings:access.sectionUnattendedAccess')}</SectionTitle>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          <div style={{ fontSize: 10, color: '#93a0b2' }}>{t('settings:access.permanentPasswordLabel')}</div>
          <span style={{ fontSize: 9, fontWeight: 600, color: '#b3bdca', background: '#21262d', border: '1px solid #30363d', borderRadius: 10, padding: '1px 7px' }}>{t('common:comingSoon')}</span>
        </div>
        <input
          type="password"
          disabled
          placeholder={t('settings:access.permanentPasswordPlaceholder')}
        ...
        />
        <div style={{ fontSize: 10, color: '#93a0b2', marginTop: 4 }}>{t('settings:access.permanentPasswordNote')}</div>
      </div>
    </div>
  );
}
